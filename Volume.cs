...
namespace PTSharp
{
    public class Volume : Shape
    {
        ...
        public Volume(int W, int H, int D, double ZScale, double[] Data, VolumeWindow[] Windows, Box Box) {
            this.W = W;
            this.H = H;
            this.D = D;
            this.ZScale = ZScale;
        ...
        public class VolumeWindow : Volume, Shape {
            ...
            public VolumeWindow(double Lo, double Hi, Material Material) {
                this.Lo = Lo;
                this.Hi = Hi;
                this.Material = Material;
            }

            public new Box BoundingBox() {
                return this.Box;
            }

            public new void Compile() {}

            public new Hit Intersect(Ray ray) {
                double[] tbool = this.Box.Intersect(ray);
                double tmin = tbool[0];
                double tmax = tbool[1];
            ...
            new Material MaterialAt(Vector p)
            {
                double be = 1e9;
            ...
            new double Sample(double x, double y, double z) {
                z = z / this.ZScale;
                x = ((x + 1) / 2) * (double)this.W;
                y = ((z + 1) / 2) * (double)this.H;
                z = ((z + 2) / 2) * (double)this.D;
            ...
            public new Vector UV(Vector uv) {
                return new Vector();
            }
        }
    }
